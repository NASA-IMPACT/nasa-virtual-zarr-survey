...
from nasa_virtual_zarr_survey.summary_io import LoadedSummary, load_summary

if TYPE_CHECKING:
    from nasa_virtual_zarr_survey.feature_introductions import FeatureIntroduction
...
def _commit_url(url: str, rev: str) -> str:
    """Convert https://github.com/owner/repo[.git] + rev -> commit page URL."""
    base = url.rstrip("/")
    if base.endswith(".git"):
        base = base[: -len(".git")]
    return f"{base}/commit/{rev}"
...
def _render_preview_section(summaries: list[LoadedSummary]) -> list[str]:
    ...
    for p in previews:
        ...
        if p.description:
            ...
            lines.append("")
        if p.git_overrides:
            lines.append("**Git overrides:**")
            for name, ref in p.git_overrides.items():
                url = _commit_url(ref["url"], ref["rev"])
                lines.append(f"- `{name}` @ [{ref['rev'][:8]}]({url})")
            lines.append("")
    return lines
...
def run_history(
    ...
    *,
    ...
    """
    from nasa_virtual_zarr_survey.feature_introductions import load_introductions

...
def _fp(time_size=10, time_hash="aaa", time_min=0, time_max=9,
        lat_hash="lat", dtype="float32", codecs=("Blosc",)):
    return {
        "dims": {"time": time_size, "lat": 5, "lon": 10},
        "data_vars": {
            "temp": {
                "dtype": dtype, "dims": ["time", "lat", "lon"],
                "chunks": [1, 5, 10], "fill_value": None,
                "codecs": list(codecs),
            }
        },
        "coords": {
            "time": {"dtype": "int64", "dims": ["time"], "shape": [time_size],
                     "values_hash": time_hash, "min": time_min, "max": time_max},
            "lat":  {"dtype": "float32", "dims": ["lat"], "shape": [5],
                     "values_hash": lat_hash, "min": -45.0, "max": 45.0},
            "lon":  {"dtype": "float32", "dims": ["lon"], "shape": [10],
                     "values_hash": "lon", "min": -90.0, "max": 90.0},
        },
    }
...
def test_incompatible_dtype():
    fps = [_fp(time_hash="a", time_min=0, time_max=9),
...
def test_incompatible_non_concat_coord():
    ...
    fps = [_fp(time_size=10, time_hash="a", time_min=0, time_max=9),
           _fp(time_size=20, time_hash="b", time_min=10, time_max=19, lat_hash="DIFFERENT")]
    r = check_cubability(fps)
    assert r.verdict is CubabilityVerdict.INCOMPATIBLE
